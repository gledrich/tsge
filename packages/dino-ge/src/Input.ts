    ...
    document.addEventListener('mousemove', (event: MouseEvent) => {
      ...
      if (Engine.debug && this.isResizing && Engine.selectedObject) {
        ...
        if (sprite && sprite.frameWidth > 0 && sprite.frameHeight > 0) {
          ...
          const shape = obj.getComponent(ShapeComponent) as any;
          const text = obj.getComponent(TextComponent) as any;

          if (shape && shape.width !== undefined) {
            shape.width = newWidth;
            shape.height = newHeight;
          } else if (text && text.width !== undefined) {
            text.width = newWidth;
